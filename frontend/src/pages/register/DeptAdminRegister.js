...
const DeptAdminRegister = () => {
  const navigate = useNavigate();
  const [formData, setFormData] = useState({
    fullName: '', gender: '', dateOfBirth: '', mobileNumber: '', email: '', password: '',
    aadhaarNumber: '', employeeId: '', departmentName: '', designation: '', officeLocation: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/users/register', {
        ...
        body: JSON.stringify({ ...formData, role: 'DEPT_ADMIN' })
      });
      if (response.ok) {
    ...
    <div className="register-page">
      ...
      <div className="register-left">
        <motion.div className="left-content" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
          ...
          <p className="subtitle">Manage • Organize • Lead</p>
          <div className="features-list">
            <div className="feature-item"><span>Manage Schemes</span></div>
            <div className="feature-item"><span>Department Operations</span></div>
            <div className="feature-item"><span>Policy Implementation</span></div>
          </div>
        </motion.div>
      </div>
      <div className="register-right">
        <motion.div className="form-container" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8 }}>
          <h2>🏛️ Department Admin Registration</h2>
          <p className="form-subtitle">Register to manage department schemes</p>
          <form onSubmit={handleSubmit} className="signup-form">
            ...
            <div className="form-section">
              ...
              <div className="form-group">
                ...
                <input type="text" name="officeLocation" value={formData.officeLocation} onChange={handleChange} required />
              </div>
            </div>
...
export default DeptAdminRegister;
