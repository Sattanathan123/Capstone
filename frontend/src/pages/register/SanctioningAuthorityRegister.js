...
const SanctioningAuthorityRegister = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/users/register', {
        ...
        body: JSON.stringify({ ...formData, role: 'SCHEME_SANCTIONING_AUTHORITY' })
      });
      if (response.ok) {
    ...
    <div className="register-page">
      ...
      <div className="register-left">
        <motion.div className="left-content" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
          <h1>Scheme Sanctioning Authority</h1>
          <p className="subtitle">Review • Approve • Sanction</p>
          <div className="features-list">
            <div className="feature-item"><span>Application Review</span></div>
            <div className="feature-item"><span>Scheme Approval</span></div>
            <div className="feature-item"><span>Fund Sanctioning</span></div>
          </div>
        </motion.div>
      </div>
      <div className="register-right">
        <motion.div className="form-container" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8 }}>
          <h2>✅ Sanctioning Authority Registration</h2>
          <p className="form-subtitle">Register to approve and sanction schemes</p>
          <form onSubmit={handleSubmit} className="signup-form">
            ...
            <div className="form-section">
              <h3>Sanctioning Authority Details</h3>
              <div className="form-row">
                <div className="form-group">
                  <label>Sanctioning Level *</label>
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  <label>Max Sanction Amount (₹) *</label>
                  <input type="number" name="maxSanctionAmount" value={formData.maxSanctionAmount} onChange={handleChange} required />
                </div>
              </div>
            </div>
...
export default SanctioningAuthorityRegister;
