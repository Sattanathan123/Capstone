                    ...
                    return (
                      <>
                        <svg className="progress-ring" width="120" height="120">
                          ...
                          <circle className="progress-ring-fill" stroke="#f093fb" strokeWidth="8" fill="transparent" r={r} cx="60" cy="60"
                ...
                <div className="performance-stats">
                  <div className="perf-stat">
                    ...
                    <div className="perf-bar">
                      <div className="perf-fill" style={{width:`${Math.min((stats.today/10)*100,100)}%`, background:'linear-gradient(90deg,#667eea,#f093fb)'}}></div>
                    </div>
                    <span className="perf-value">{stats.today}/10 daily target</span>
                  </div>
                  <div className="perf-stat">
                    <span className="perf-label">Pending Workload</span>
                    <div className="perf-bar">
                      <div className="perf-fill workload" style={{width:`${Math.min((stats.pending/50)*100,100)}%`}}></div>
                    </div>
                    <span className="perf-value">{stats.pending} pending sanctions</span>
                  </div>
                  <div className="perf-stat">
                    <span className="perf-label">Sanctioned vs Rejected</span>
                    <div className="perf-bar">
                      <div className="perf-fill" style={{width:`${stats.sanctioned+stats.rejected>0?Math.round(stats.sanctioned/(stats.sanctioned+stats.rejected)*100):0}%`, background:'linear-gradient(90deg,#48bb78,#38a169)'}}></div>
                    </div>
              ...
              {selectedApp.status === 'APPROVED' && (
                <>
                  ...
                  <div className="action-buttons">
                    <button className="btn-approve" onClick={() => { const remarks = document.getElementById('remarks').value; handleSanction(selectedApp.id, 'SANCTIONED', remarks); }}>
                      ✅ {t('common.sanction')}
                    </button>
                    <button className="btn-reject" onClick={() => { const remarks = document.getElementById('remarks').value; if (!remarks) { alert('Please provide remarks for rejection'); return; } handleSanction(selectedApp.id, 'REJECTED', remarks); }}>
                      ❌ {t('common.reject')}
                    </button>
